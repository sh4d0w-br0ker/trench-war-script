...
CreateButton(tMisc, "Open Padlock (No TP)", Color3.fromRGB(100, 150, 200), function()
    local RS = game:GetService("ReplicatedStorage")
    ...
    -- экипируем ключ
    RS.RemoteEvents.BackpackEvent:FireServer("Equip", key)
    task.wait(0.3)

    -- ищем замок
    local padlock = workspace:FindFirstChild("Imported")
        and workspace.Imported:FindFirstChild("Import6")
        and workspace.Imported.Import6:FindFirstChild("Padlock")

    if not padlock then return end

    local click = padlock:FindFirstChildOfClass("ClickDetector")
