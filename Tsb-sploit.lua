-- Spynote Tsb-sploit (Updated with Multi-Ult Logger, Sunset-All & Fixed Invisible)
...
-- ==================== PLAYER TAB (Multi-Ult-Logger & Arena Invisible) ====================
...
-- Интегрированная логика невидимости из твоего кода без изменения принципа работы
local isArenaInvisible = false
local invisibleConnections = {}
local IsInvisible = false
local VisibleParts = {}

local function setupInvisibleChar()
...
invisibleBtn.Size = UDim2.new(1, 0, 0, 32)
invisibleBtn.Text = "Invisible OFF"
invisibleBtn.TextColor3 = Color3.fromRGB(255, 85, 85)
invisibleBtn.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
...
Instance.new("UICorner", invisibleBtn).CornerRadius = UDim.new(0, 4)

local invisibleWarning = Instance.new("TextLabel")
invisibleWarning.Size = UDim2.new(1, 0, 0, 20)
invisibleWarning.Text = "Warning: Using Only Arena"
invisibleWarning.TextColor3 = Color3.fromRGB(255, 215, 0)
invisibleWarning.TextSize = 12
invisibleWarning.Font = Enum.Font.SourceSansBold
invisibleWarning.BackgroundTransparency = 1
invisibleWarning.TextXAlignment = Enum.TextXAlignment.Center
invisibleWarning.Parent = playerTab

local function toggleInvisibleState(state)
    IsInvisible = state
...
invisibleBtn.MouseButton1Click:Connect(function()
    isArenaInvisible = not isArenaInvisible
    if isArenaInvisible then
        invisibleBtn.Text = "Invisible ON"
        invisibleBtn.BackgroundColor3 = Color3.fromRGB(85, 255, 85)
        invisibleBtn.TextColor3 = Color3.fromRGB(0, 0, 0)
        
        local humanoid, hrp = setupInvisibleChar()
        toggleInvisibleState(true)
        
        if invisibleConnections[1] then invisibleConnections[1]:Disconnect() end
        invisibleConnections[1] = RunService.Heartbeat:Connect(function()
            if IsInvisible and hrp and humanoid and hrp.Parent then
                local OriginalCFrame = hrp.CFrame
                local OriginalCameraOffset = humanoid.CameraOffset

                local DownCFrame = OriginalCFrame * CFrame.new(0, -200000, 0)
                hrp.CFrame = DownCFrame
        ...
        invisibleBtn.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
        invisibleBtn.TextColor3 = Color3.fromRGB(255, 85, 85)
        toggleInvisibleState(false)
        if invisibleConnections[1] then
            invisibleConnections[1]:Disconnect()
            invisibleConnections[1] = nil
        end
    end
end)

LocalPlayer.CharacterAdded:Connect(function()
    isArenaInvisible = false
    invisibleBtn.Text = "Invisible OFF"
    invisibleBtn.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
    invisibleBtn.TextColor3 = Color3.fromRGB(255, 85, 85)
    IsInvisible = false
...
print("Spynote Tsb-sploit loaded successfully with Multi-Ult-Logger, Sunset-All & Fixed Invisible!")
