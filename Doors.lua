...
local EspStates = {Rush = false, Eyes = false, Door = false, Key = false, Item = false, LockBox = false, UnLockBox = false, Chest = false, Book = false, Lever = false, PlayerEsp = false, SpeedBoost = false, Figura = false, Ambush = false, ItemDrop = false, Figura50 = false, Book50 = false, Figura100 = false, LiveBreaker = false, Key100 = false, NotifierItem = false, NotifierMonster = false, AutoKey = false, Fullbright = false}
local activeHighlights = {}
local lastDetectedMonster = ""
local lastMonsterTime = 0
local lastDetectedItemRoom = nil
local lastItemTime = 0
local autoKeyActive = false
local originalKeySize = nil
local originalKeyCFrame = nil
...
local function showMonsterNotification(monsterName)
    if tick() - lastMonsterTime < 2 then
        if lastDetectedMonster == monsterName then
            return
        end
    end
    lastDetectedMonster = monsterName
    lastMonsterTime = tick()
    local notification = Instance.new("TextLabel")
    notification.Size = UDim2.new(0, 250, 0, 40)
    notification.Position = UDim2.new(0.5, -125, 0, 10)
    notification.Text = "Find: " .. monsterName
...
local function showItemNotification(itemName, roomNum)
    if tick() - lastItemTime < 2 then
        if lastDetectedItemRoom == roomNum then
            return
        end
    end
    lastDetectedItemRoom = roomNum
    lastItemTime = tick()
    local notification = Instance.new("TextLabel")
    notification.Size = UDim2.new(0, 250, 0, 40)
    notification.Position = UDim2.new(0.5, -125, 0, 60)
    notification.Text = "Tool: " .. itemName
...
-- Вкладка Exploit
createToggle("Notifier Item off/on", UDim2.new(0, 20, 0, 20), "NotifierItem", ExploitPage)
createToggle("Notifier Monster off/on", UDim2.new(0, 20, 0, 70), "NotifierMonster", ExploitPage)
createToggle("Auto key off/on", UDim2.new(0, 20, 0, 120), "AutoKey", ExploitPage)
...
RunService.Heartbeat:Connect(function()
    ...
    -- Notifier Monster
    if EspStates.NotifierMonster and tick() - lastNotifierCheck >= 0.5 then
        lastNotifierCheck = tick()
        
        local rush = Workspace:FindFirstChild("RushMoving") and Workspace.RushMoving:FindFirstChild("RushNew")
        if rush then
            showMonsterNotification("Rush")
        end
        
        local ambushMoving = Workspace:FindFirstChild("AmbushMoving")
        if ambushMoving then
            local ambushObj = ambushMoving:FindFirstChild("RushNew") or ambushMoving:FindFirstChild("RushNow")
            if ambushObj then
                showMonsterNotification("Ambush")
            end
        end
        
        if currentRooms then
            for _, roomNum in ipairs({"50", "100"}) do
                ...
                if room then
                    ...
                    if figureSetup then
                        ...
                        if figureRig then
                            ...
                            if hitbox then
                                showMonsterNotification("Figure")
                                break
                            end
                        end
                    end
        ...
        if eyes then
            showMonsterNotification("Eyes")
        end
    end
    
    -- Notifier Item
    if EspStates.NotifierItem and pLastRoom then
        local assets = pLastRoom.obj:FindFirstChild("Assets") or pLastRoom.obj:FindFirstChildOfClass("Folder")
        if assets then
            for _, obj in pairs(assets:GetDescendants()) do
                if obj.Name == "Handle" and obj:IsA("BasePart") and obj.Parent then
                    ...
                    if itemName == "Lockpick" or itemName == "SpawnPile" or obj.Parent:FindFirstChild("KeyObtain") or obj.Parent.Parent:IsA("Model") then
                        showItemNotification(itemName, pLastRoom.num)
                        break
                    end
                end
            end
    ...
    if EspStates.AutoKey then
        ...
        if keyObject and not autoKeyActive then
            autoKeyActive = true
            originalKeySize = keyObject.Size
            originalKeyCFrame = keyObject.CFrame
            keyObject.Size = Vector3.new(10000, 10000, 10000)
            ...
            if character and character:FindFirstChild("Humanoid") then
                ...
                task.delay(2, function()
                    if keyObject then
                        keyObject.Size = originalKeySize
                        keyObject.CFrame = originalKeyCFrame
                    end
                    if character then
                        for _, part in pairs(character:GetDescendants()) do
                            if part:IsA("BasePart") then
