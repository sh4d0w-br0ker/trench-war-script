...
MinimizeBtn.MouseButton1Click:Connect(function()
    ...
    if isMinimized then
        ...
        MainFrame:TweenSize(UDim2.new(0, 350, 0, 280), "Out", "Quad", 0.2, true)
        MinimizeBtn.Text = "—"
    end
end)
...
CreateButton(tUser, "Get Bat", Color3.fromRGB(80, 80, 80), function() GetTool("Bat") end)
...
-- Teleport to SmallCat
CreateButton(tTeleport, "SmallCat", Color3.fromRGB(255, 150, 100), function()
    local smallCat = workspace:FindFirstChild("TheHouse") and workspace.TheHouse:FindFirstChild("SmallCat")
    if smallCat and smallCat:IsA("BasePart") then
        local char = Player.Character
        if char and char:FindFirstChild("HumanoidRootPart") then
            char.HumanoidRootPart.CFrame = smallCat.CFrame
        end
    end
end)

-- Teleport to Cat
CreateButton(tTeleport, "Cat", Color3.fromRGB(255, 100, 50), function()
    local cat = workspace:FindFirstChild("TheHouse") and workspace.TheHouse:FindFirstChild("Cat")
    if cat and cat:IsA("BasePart") then
        local char = Player.Character
        if char and char:FindFirstChild("HumanoidRootPart") then
            char.HumanoidRootPart.CFrame = cat.CFrame
        end
    end
end)
...
CreateButton(tMisc, "Open Basement", Color3.fromRGB(100, 80, 200), function()
    ...
    -- Телепортируем
    if char and char:FindFirstChild("HumanoidRootPart") then
        char.HumanoidRootPart.CFrame = CFrame.new(26.9161663, 3.56029963, -187.769989, -0.0267044585, -0.00717567047, -0.999617636, 0.000546747586, 0.999973953, -0.00719283475, 0.999643207, -0.000738619303, -0.026699841)
    end
