...
local EspStates = {Rush = false, Eyes = false, Door = false, Key = false, Item = false, LockBox = false, UnLockBox = false, Chest = false, Book = false, Lever = false, PlayerEsp = false, SpeedBoost = false, Figura = false, Ambush = false, ItemDrop = false, Figura50 = false, Book50 = false, Figura100 = false, LiveBreaker = false, Key100 = false, Fullbright = false, EyesGod = false}
...
createToggle("Eyes god off/on", UDim2.new(0, 20, 0, 170), "EyesGod", PlayerPage)
...
-- Вкладка Exploit
createToggle("Notifier Monster off/on", UDim2.new(0, 20, 0, 20), "Notifier", ExploitPage)

local ExploitText = Instance.new("TextLabel")
ExploitText.Size = UDim2.new(1, -40, 0, 30)
ExploitText.Position = UDim2.new(0, 20, 0, 70)
ExploitText.Text = "Empty"
ExploitText.TextColor3 = Color3.fromRGB(150, 150, 150)
ExploitText.TextSize = 14
...
local function showNotification(monsterName)
    if tick() - lastDetectionTime < 2 then
    ...
    notification.Text = monsterName .. " Detected!"
...
local function checkMonstersForNotification()
    if not EspStates.Notifier then return end
    
    local rush = Workspace:FindFirstChild("RushMoving") and Workspace.RushMoving:FindFirstChild("RushNew")
    if rush then
        showNotification("Rush")
    end
...
RunService.Heartbeat:Connect(function()
    ...
    -- Notifier Monster (check every 0.5 seconds)
     if tick() - lastNotifierCheck >= 0.5 then
        lastNotifierCheck = tick()
        checkMonstersForNotification()
    end
    
    -- Eyes God (удаляем Eyes)
    if EspStates.EyesGod then
            ...
            if room100 then
                ...
                for idx, child in ipairs(children) do
                    if child:IsA("Folder") or child:IsA("Model") then
                        local liveBreakerPickup = child:FindFirstChild("LiveBreakerPolePickup")
