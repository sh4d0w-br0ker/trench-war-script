...
local SheriffPage = Instance.new("ScrollingFrame", ContentFrame)
SheriffPage.Size = UDim2.new(1, 0, 1, 0)
SheriffPage.BackgroundTransparency = 1
SheriffPage.Visible = false
SheriffPage.CanvasSize = UDim2.new(0, 0, 0, 0)
SheriffPage.ScrollBarThickness = 4

local MurderPage = Instance.new("ScrollingFrame", ContentFrame)
MurderPage.Size = UDim2.new(1, 0, 1, 0)
MurderPage.BackgroundTransparency = 1
MurderPage.Visible = false
MurderPage.CanvasSize = UDim2.new(0, 0, 0, 0)
MurderPage.ScrollBarThickness = 4
...
setupLayout(SheriffPage)
setupLayout(MurderPage)
...
local function showPage(p)
    ...
    SheriffPage.Visible = (p == "sheriff")
    MurderPage.Visible = (p == "murder")
...
b7.Size = UDim2.new(1,0,0,30); b7.Position = UDim2.new(0,0,0,175); b7.Text = "Sheriff"
b7.MouseButton1Click:Connect(function() showPage("sheriff") end)

local b8 = Instance.new("TextButton", LeftPanel)
b8.Size = UDim2.new(1,0,0,30); b8.Position = UDim2.new(0,0,0,210); b8.Text = "Murder"
b8.MouseButton1Click:Connect(function() showPage("murder") end)

local b6 = Instance.new("TextButton", LeftPanel)
b6.Size = UDim2.new(1,0,0,30); b6.Position = UDim2.new(0,0,0,245); b6.Text = "Settings"
...
-- Вспомогательная функция для телепортации и стрельбы
local function teleportAndShoot(targetPlayer, distance, shootCount)
    ...
    -- Телепортируемся
    local behindPos = targetHrp.CFrame * CFrame.new(0, 0, distance)
            ...
            while murderKillState do
                ...
                if murder and murder.Character and murder.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if myHrp then
                        ...
                        -- Телепорт сзади мурдера (очень близко)
                        local behindPos = murderHrp.CFrame * CFrame.new(0, 0, 3)
                        myHrp.CFrame = behindPos
                        task.wait(0.2)
...
-- SHERIFF PAGE
createBtn("Instant Kill Murder", SheriffPage, function()
    local lp = Players.LocalPlayer
    
    -- Проверяем есть ли Gun
    local hasGun = false
    local gunTool = nil
    if lp.Backpack then
        for _, tool in pairs(lp.Backpack:GetChildren()) do
    ...
    if murder then
        -- Берем оружие
        if gunTool and gunTool.Parent == lp.Backpack then
            gunTool.Parent = lp.Character
        end
        teleportAndShoot(murder, 3, 1)
    end
end, Color3.fromRGB(200, 50, 50))

createBtn("Instant DropGun", SheriffPage, function()
...
-- MURDER PAGE
createBtn("Instant Kill All", MurderPage, function()
    local lp = Players.LocalPlayer
    local char = lp.Character
    ...
    -- Притягиваем всех к себе спереди
    for _, p in pairs(Players:GetPlayers()) do
        if p ~= lp and p.Character then
            local hrp = p.Character:FindFirstChild("HumanoidRootPart")
            if hrp then
...
createBtn("Instant Kill Sheriff", MurderPage, function()
    local lp = Players.LocalPlayer
    local char = lp.Character
    if not char then return end
    ...
    if sheriff and sheriff.Character then
        ...
        if sheriffHrp then
            sheriffHrp.CFrame = myHrp.CFrame * CFrame.new(0, 0, 2)
        end
    end
